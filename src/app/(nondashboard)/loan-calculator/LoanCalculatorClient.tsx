      ...
      <section className="relative bg-gradient-to-r from-[#00acee] to-[#004d71] py-16 md:py-20">
        <div className="container mx-auto px-4 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-lg text-blue-100 max-w-2xl mx-auto mb-8">Estimate your monthly vehicle installment in South Africa and explore finance scenarios with AutoFame.</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <Card className="p-6 border dark:border-gray-800">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    Down Payment (Deposit): {formatCurrency(downPayment[0])}
                  </Label>
                  <Slider
                    value={downPayment}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 dark:text-gray-400">Principal Financed</div>
                      <div className="text-lg font-semibold text-gray-900 dark:text-white">{formatCurrency(loanAmount[0] - downPayment[0])}</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 dark:text-gray-400">Total Interest</div>
                      <div className="text-lg font-semibold text-gray-900 dark:text-white">{formatCurrency(totalInterest)}</div>
                    </div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                    <div className="text-sm text-gray-600 dark:text-gray-400">Total Payment Amount</div>
                    <div className="text-lg font-semibold text-gray-900 dark:text-white">{formatCurrency(totalPayment)}</div>
                  </div>
                  <Button asChild className="w-full bg-[#00acee] hover:bg-[#004d71] text-white">
                    <a href="/financing">Proceed to Finance Application <ArrowRight className="ml-2 h-4 w-4" /></a>
                  </Button>
                </div>
              </CardContent>
            </Card>
